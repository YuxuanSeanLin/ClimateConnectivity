...
for (hemi in c('N', 'S')){
  files <- list.files(paste0('step6_patches_hemi/',hemi,'_centro'), 
  ...
  for (f in files){
    ...
    # export patches
    to_path <- paste0('step7_patches_hemi_final/',hemi,'_centro_patch/',
                      depth,strsplit(f,depth)[[1]][2],depth,
                      strsplit(f,depth)[[1]][3])
