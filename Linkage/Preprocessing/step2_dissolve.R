...
setwd('D:/Users/Yuxuan Lin/Documents/LocalFiles/XMU/Connectivity/Linkage/patchtif/test0110')

# patch
files <- list.files('patches_id2rs', pattern = '.tif$', recursive = T, full.names = T)
for (f in files){
  # class
  f1 = paste0('patches_raw/', strsplit(f,'patches_id2rs/')[[1]][2])
  ...
  # export shp
  to_path <- paste0(strsplit(paste0('patches_newid/',strsplit(f,'patches_id2rs/')[[1]][2]),'.tif')[[1]][1], '.shp')
  st_write(sf.poly, to_path)
}
